"""
==================================
Cartesian DMP on UR5 with MIA hand
==================================

A trajectory is created manually, imitated with a Cartesian DMP, converted
to a joint trajectory by inverse kinematics, and executed with a UR5 where
the end-effector location is a point between the palm and the thumb (grasp_location_indicator).
"""
print(__doc__)
...
with open("examples/data/urdf/ur5_with_left_mia.urdf", "r") as f:
...
chain = kin.create_chain(
    ...
    "ur5_base_link", "grasp_location_indicator")

q0 = np.deg2rad([-170, -120, -80, 200, -90 + 30, 0])
chain.forward(q0)

ee2base_start = kin.tm.get_transform("grasp_location_indicator", "ur5_base_link")
...
graph = fig.plot_graph(
    kin.tm, "ur5_base_link", show_visuals=True, show_collision_objects=False,
    show_frames=True, s=0.1, whitelist=["ur5_base_link", "grasp_location_indicator"])
